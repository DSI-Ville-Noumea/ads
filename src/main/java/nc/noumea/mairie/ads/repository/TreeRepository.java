...
public class TreeRepository implements ITreeRepository {
	...
	public List<Noeud> getWholeTreeForRevision(long idRevision) {
		...
		queryBuilder.append("WITH RECURSIVE ads_tree_walker(id_noeud, id_service, sigle, label, id_revision, id_noeud_parent, id_type_noeud, version) AS ( ");
		queryBuilder.append("SELECT an.id_noeud, an.id_service, an.sigle, an.label, an.id_revision, an.id_noeud_parent, an.id_type_noeud, an.version FROM ads_noeud an ");
		queryBuilder.append("WHERE an.id_revision = :idRevision ");
		queryBuilder.append("UNION ALL ");
		queryBuilder.append("SELECT an.id_noeud, an.id_service, an.sigle, an.label, an.id_revision, an.id_noeud_parent, an.id_type_noeud, an.version FROM ads_noeud an, ads_tree_walker ");
